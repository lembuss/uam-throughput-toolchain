...
def update_worksheet(sheet, class_column, cat_column):
    for row_number, cell in enumerate(sheet.iter_rows(min_row=5, min_col=3, max_col=3, values_only=True), start=5):
        if cell:
            ...
            sheet.cell(row=row_number, column=class_column, value=aircraft_class)
            sheet.cell(row=row_number, column=cat_column, value=aircraft_category)
...
for sheet_name in traffic_sheets:
    ...
    class_column = 5
    cat_column = 6
    update_worksheet(sheet, class_column, cat_column)
